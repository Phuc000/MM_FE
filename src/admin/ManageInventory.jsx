...
const ManageInventory = () => {
    ...
    axios
      ...
      .then(() => {
        ...
        setInventoryData((prevData) =>
          prevData.map((item) =>
            item.productID === updatedRecord.productID && item.storeID === updatedRecord.storeID
  ...
  const handleDeleteInventory = (productID, storeID) => {
    ...
    if (window.confirm(`Are you sure you want to remove ${productName} from inventory?`)) {
      ...
      axios
        .delete(`${import.meta.env.VITE_REACT_APP_API_URL}/products/${productID}/${storeID}`)
        .then(() => {
          // Update the inventoryData state
          setInventoryData((prevData) =>
            prevData.filter((item) => !(item.productID === productID && item.storeID === storeID)),
          );
    ...
    axios
      ...
      .then(() => {
        ...
        const existingRecord = inventoryData.find(
          (item) => item.productID === newRecord.productID && item.storeID === newRecord.storeID,
        );
        if (existingRecord) {
          // Update the existing record's quantity
          setInventoryData((prevData) =>
            prevData.map((item) =>
              item.productID === newRecord.productID && item.storeID === newRecord.storeID
                ? {
                    ...item,
                    numberAtStore:
                      parseInt(item.numberAtStore, 10) + parseInt(newRecord.numberAtStore, 10),
                  }
                : item,
            ),
          );
