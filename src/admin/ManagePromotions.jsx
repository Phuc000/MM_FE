...
const ManagePromotions = () => {
  ...
  const handleDeletePromotion = async (type, promotionId) => {
    try {
      await axios.delete(`${import.meta.env.VITE_REACT_APP_API_URL}/promotions/${type}/${promotionId}`,
        {
          headers: {
            'Content-Type': 'application/json',
          },
      ...
      setPromotions((prev) =>
        prev.filter((promo) =>
          (promo.promotionId || promo.promotionID || promo.id) !== promotionId
        )
      );
    ...
    <Card sx={{ mb: 3, mx: { xs: 1, sm: 0 } }}>
      <CardContent sx={{pb:0}}>
        ...
        {promotion.type === 'Bill Promotion' && (
          <Typography variant="body2" sx={{ mt: 1 }}>
            Apply Price: ${promotion.applyPrice.toFixed(2)}<br />
        ...
        <Typography variant="body2" sx={{ mt: 1 }}>
          Start: {new Date(promotion.startDay).toLocaleDateString()}<br />
          End: {new Date(promotion.endDay).toLocaleDateString()}
        </Typography>
        <Typography variant="body2" sx={{ mt: 1 }}>{promotion.description}</Typography>
      </CardContent>
        ...
        <TableBody>
          {loading ? (
            ...
            promotions.map((promotion) => (
              <PromotionCard key={promotion.promotionId || promotion.promotionID || promotion.id} promotion={promotion} />
            ))
          ) : (
            // Desktop: Table view
            promotions.map((promotion) => (
              <TableRow key={promotion.promotionId || promotion.promotionID || promotion.id}>
                <TableCell>{promotion.promotionId || promotion.promotionID || promotion.id}</TableCell>
                ...
                <TableCell align="right">
                  <IconButton color="error" onClick={() => handleDeletePromotion(promotion.type, promotion.promotionId || promotion.promotionID || promotion.id)}>
                    <DeleteIcon />
                  </IconButton>
                </TableCell>
              </TableRow>
