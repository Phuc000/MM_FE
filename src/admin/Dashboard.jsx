    ...
    const fetchData = async () => {
      try {
        ...
        const [transactionsRes, storesRes] = await Promise.all([
          axios.get(`${import.meta.env.VITE_REACT_APP_API_URL}/transactions`, {
        ...
        const totalRev = transactionsData.reduce((sum, tx) => sum + tx.totalPrice, 0);

        // Calculate revenue per store
        const revenueStore = storesData.map((store) => {
          const storeTransactions = transactionsData.filter((tx) => tx.storeID === store.storeID);
          const storeRevenue = storeTransactions.reduce((sum, tx) => sum + tx.totalPrice, 0);
          return {
                  ...
                  .map((tx) => {
                    ...
                    return (
                      <TableRow key={tx.transactionId}>
                        <TableCell>{new Date(tx.dateAndTime).toLocaleDateString()}</TableCell>
