  ...
  const handleShopRecipeConfirm = async (shopRecipeItems) => {
    try {
      ...
      for (const item of selectedItems) {
        await addToCart(userId, item.product.id, item.product.quantity, locationContext);
      }
  ...
  useEffect(() => {
    ...
    const connectWebSocket = () => {
      ...
      ws.onmessage = async (event) => {
        ...
        const recipeActions =
          data.actions?.filter((action) => action.action === "view_recipe") || [];
        ...
        if (data.actions && data.actions.length > 0) {
          ...
          const recipePromises = recipeActions.map((action) => handleViewRecipe(action.recipe_id));
