...
const Dashboard = () => {
    ...
    const fetchShipperOrders = async () => {
      try {
        ...
        const filteredOrders = response.data.filter(
          (tx) => tx.shipperID === user.id && (tx.deliveryStatus === 2 || tx.deliveryStatus === 3),
        );

        // Calculate total weight
        const totalWeight = filteredOrders.reduce((sum, tx) => sum + tx.totalWeight, 0);
  ...
  const handleUpdateStatus = async (transactionId, newStatus) => {
    try {
      ...
      setShipperOrders((prev) =>
        prev.map((tx) =>
          tx.transactionId === transactionId ? { ...tx, deliveryStatus: newStatus } : tx,
        ),
      );
  ...
  const handleReschedule = async (transactionId) => {
    try {
      ...
      // Update local state
      setShipperOrders((prev) =>
        prev.map((tx) => (tx.transactionId === transactionId ? { ...tx, deliveryStatus: 2 } : tx)),
      );
  ...
  const handleCancel = async (transactionId) => {
    try {
      ...
      setShipperOrders((prev) => prev.filter((tx) => tx.transactionId !== transactionId));
          ...
          >
            ...
            >
              <Typography variant="subtitle1" gutterBottom sx={{ mb: 0, fontWeight: "medium" }}>
                {storeRoute.storeName} - Total Distance: {(storeRoute.distance / 1000).toFixed(2)}{" "}
                km
              </Typography>
              <IconButton size="small">
                {openRoutes[storeRoute.storeId] ? (
                  <KeyboardArrowUpIcon />
                ) : (
            ...
            <Collapse in={openRoutes[storeRoute.storeId]} timeout="auto" unmountOnExit>
              <TableContainer
                component={Paper}
                sx={{ borderTop: "1px solid #e0e0e0", borderRadius: "0 0 4px 4px" }}
              >
                <Table size="small">
                  <TableHead>
                    <TableRow>
  ...
  const orderPercentage =
    shipperOrders.length > 0 ? Math.min((shipperOrders.length / 15) * 100, 100) : 0;
  ...
  return (
    <Box>
      ...
      {shipperInfo && (
        <Box mb={2}>
          <Typography variant="h6">Order capacity: {shipperOrders.length} / 15 Orders</Typography>
      ...
      <Dialog open={bombConfirmation.open} onClose={handleBombConfirmClose}>
